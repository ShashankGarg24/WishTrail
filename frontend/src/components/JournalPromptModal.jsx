          ...
          {!llmResult ? (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Write 1-3 sentences..." rows={4} className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 resize-none" />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {moods.map(m => (
                        <button type="button" key={m.value} onClick={() => setMood(m.value)} className={`px-3 py-2 rounded-lg border text-sm flex items-center gap-2 ${mood === m.value ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300' : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300'}`}>{m.icon}{m.label}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {visibilityOptions.map(v => (
                        <button type="button" key={v.value} onClick={() => setVisibility(v.value)} className={`px-3 py-2 rounded-lg border text-sm flex items-center gap-2 ${visibility === v.value ? 'border-sky-500 bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-300' : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300'}`}>{v.icon}{v.label}</button>
                      ))}
                    </div>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <button type="submit" disabled={submitting || !content.trim() || words > maxWords} className="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed">{submitting ? 'Saving...' : 'Save'}</button>
                </div>
              </form>
            </>
          ) : (
