...
export default function HabitsPanel({ onCreate, onOpenHabit, scrollable = false, initialVisible = 3 }) {
  ...
  const containerClass = scrollable ? 'max-h-[420px] overflow-auto pr-1' : '';
      ...
      {habits.length === 0 ? (
        ...
        <div className={containerClass}>
          <div className="space-y-3">
            {(scrollable ? habits.slice(0) : habits).map((h, idx) => (
              <motion.div key={h._id || idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: idx * 0.03 }} className="p-4 bg-gray-50 dark:bg-gray-800/40 rounded-xl border border-gray-200 dark:border-gray-700 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 relative">
                  {isScheduledToday(h) ? (
                    (funBurst && funBurst.id === h._id) && (
                      <motion.div key={`${h._id}-${funBurst.time}`} initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1.1, opacity: 1 }} transition={{ duration: 0.45 }} className="text-orange-500" title="Great job!">
